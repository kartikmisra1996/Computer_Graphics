...
public class Parser {
	...
	public static Scene createScene(Node dataNode) {
		...
		for ( int i = 0; i < nodeList.getLength(); i++ ) {
			...
			} else if (nodeName.equalsIgnoreCase("areaLight")) {
				AreaLight areaLight = Parser.createAreaLight(n);
				scene.areaLights.put( areaLight.name, areaLight);
	...
	/**
	 * Create am area light.
	 */
	public static AreaLight createAreaLight(Node dataNode) {
		AreaLight areaLight = new AreaLight();
		areaLight.name = dataNode.getAttributes().getNamedItem("name").getNodeValue();        
		Node colorAttr = dataNode.getAttributes().getNamedItem("color");
		if ( colorAttr != null ) {
			Scanner s = new Scanner( colorAttr.getNodeValue());
			float r = s.nextFloat();
		...
		if ( fromAttr != null ) {
			...
			double y = s.nextDouble();
			double z = s.nextDouble();
			areaLight.from.set(x,y,z); 
			s.close();
		}
		Node powerAttr = dataNode.getAttributes().getNamedItem("power");
		if ( powerAttr != null ) {
			areaLight.power = Double.parseDouble( powerAttr.getNodeValue() );
		}
		Node typeAttr = dataNode.getAttributes().getNamedItem("type");
		if ( typeAttr != null ) {
			areaLight.type = typeAttr.getNodeValue();
		}        
		return areaLight;
	}
	...
	public static Material createMaterial(Node dataNode) {
		...
		if( refAttr  != null ) {
	    	...
	    	Node glazedAttr = dataNode.getAttributes().getNamedItem("glossy");
	    	if ( glazedAttr != null ) {
	    		material.glazed = Boolean.parseBoolean( glazedAttr.getNodeValue() );
	    	}
	    	Node emissionAttr = dataNode.getAttributes().getNamedItem("emission");
	    	if(emissionAttr != null) {
	    		Scanner s = new Scanner (emissionAttr.getNodeValue());
	    		float r = s.nextFloat();
	    		float g = s.nextFloat();
	    		float b = s.nextFloat();
	    		float a = (s.hasNextFloat() ? s.nextFloat() : 1);
	    		material.emission.set(r,g,b,a);
	    		s.close();
	    	}
	    	Node colourAttr = dataNode.getAttributes().getNamedItem("colour");
	    	if(colourAttr != null) {
	    		Scanner s = new Scanner (colourAttr.getNodeValue());
	    		float r = s.nextFloat();
	    		float g = s.nextFloat();
	    		float b = s.nextFloat();
	    		float a = (s.hasNextFloat() ? s.nextFloat() : 1);
	    		material.colour.set(r,g,b,a);
	    		s.close();
	    	}
		}
		return material;
